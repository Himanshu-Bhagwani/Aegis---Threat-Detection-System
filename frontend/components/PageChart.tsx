...
export type ProfileRow = {
  name: string;
  GPS: number; Login: number; Password: number; Fraud: number; Breach: number;
};
    ...
    <div style={{ width: "100%", height: 260 }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={rows} margin={{ top: 4, right: 8, left: 0, bottom: 4 }} barSize={10} barCategoryGap="25%">
          ...
          <Bar dataKey="Password" fill={C.password} radius={[3,3,0,0]} />
...
export function HourRiskChart({ highlightHour }: { highlightHour?: number }) {
  return (
    <div style={{ width: "100%", height: 180 }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={HOUR_DATA} margin={{ top: 4, right: 4, left: 0, bottom: 4 }} barSize={10}>
          <CartesianGrid strokeDasharray="3 3" stroke={C.grid} vertical={false} />
          ...
          <Tooltip {...TT} formatter={(v: number) => [`${v}%`, "Anomaly Risk"]} />
          ...
          <Bar dataKey="risk" radius={[3,3,0,0]} maxBarSize={14}>
            {HOUR_DATA.map((d, i) => (
              <Cell key={i} fill={RISK_COLOR(d.risk)} fillOpacity={highlightHour !== undefined && i === highlightHour ? 1 : 0.75} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
